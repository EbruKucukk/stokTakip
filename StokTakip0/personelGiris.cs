...
namespace StokTakip0
{
    public partial class personelGiris : Form
    {
        ...
        private void btnGirişYap_Click_1(object sender, EventArgs e)
        {
            con = new SqlConnection(@"Data Source=DESKTOP-SN77AA1;Initial Catalog=stokTakip;Integrated Security=True");
            string sorgu = "SELECT * FROM personel where personelAd=@user AND personelSoyad=@surname AND personelSifre=@pass";
            ...
            if (dr.Read())
            {
                personelMain personelmain = new personelMain();
                personelmain.Show();
                this.Hide();
                Save_Data();
            }
