...
namespace StokTakip0
{
    public partial class YöneticiGiris : Form
    {
        ...
        SqlConnection baglanti = new SqlConnection(@"Data Source = AYLINMERCAN\USERS; Initial Catalog = stokTakip; Integrated Security = True");
        ...
        private void btnGirişYap_Click(object sender, EventArgs e)
        {
            ...
            string sorgu = "SELECT * FROM yönetici where yöneticiAd=@user AND yöneticiSifre=@pass and yöneticiSoyad=@surname";
            ...
            if (dr.Read())
            {
                YöneticiMain yöneticimain = new YöneticiMain();
                yöneticimain.Show();
                this.Hide();
                Save_Data();
